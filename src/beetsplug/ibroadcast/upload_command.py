...
class IBUploadCommand(Subcommand, IBroadcastBase):
    def __init__(self, plugin):
        ...
        self.parser = OptionParser(usage="beet ib-upload [options] [QUERY...]")
        ...
        self.parser.add_option(
            ...
            dest="force",
            default=False,
            help="uploads all matched files, even if they were already uploaded",
        )
        ...
        self.parser.add_option(
            ...
            dest="pretend",
            default=False,
            help="report which files would be uploaded, but don't upload them",
        )
        ...
        super(IBUploadCommand, self).__init__(
            ...
            aliases=[common.plg_ns["__UPLOAD_ALIAS__"]]
            if common.plg_ns["__UPLOAD_ALIAS__"]
            else [],
    ...
    def upload(self, item, force=False):
        ...
        if force or self._needs_upload(item):
            try:
                new_trackid = self.ib.upload(
                    syspath(item.path), label=displayable_path(item.path), force=force
                )
                ...
                if tid:
                    ...
                    try:
                        ...
                        self.plugin._log.error(
                            f"Error trashing previously uploaded iBroadcast track {tid}."
                        )
                        self._stack_trace(e)
    ...
    def _sync_tags(self, tid, item):
        ...
        for tagid in locally_added:
            self.plugin._log.debug(
                f"--> Adding remote tag '{self._tagname(tagid)}' [{tagid}]"
            )
            ...
            try:
                ...
                self.plugin._log.error(
                    f"Error tagging iBroadcast track {tid} with tag '{self._tagname(tagid)}' [{tagid}]."
                )
                self._stack_trace(e)

        for tagid in locally_removed:
            self.plugin._log.debug(
                f"--> Removing remote tag '{self._tagname(tagid) or '[deleted tag]'}' [{tagid}]"
            )
            try:
                ...
                self.plugin._log.error(
                    f"Error untagging iBroadcast track {tid} with tag '{self._tagname(tagid)}' [{tagid}]."
                )
                self._stack_trace(e)

        for tagid in remotely_added:
            self.plugin._log.debug(
                f"--> Adding local tag '{self._tagname(tagid)}' [{tagid}]"
            )
            lastsync_tagids.add(tagid)

        for tagid in remotely_removed:
            self.plugin._log.debug(
                f"--> Removing local tag '{self._tagname(tagid) or '[deleted tag]'}' [{tagid}]"
            )
    ...
    def _local_tagids(self, item):
        ...
        return (
            [self._tagid(tagname) for tagname in usertags.split("|")]
            if usertags
            else []
        )
    ...
    def _lastsync_tagids(item):
        return (
            item.ib_tagids.split("|")
            if hasattr(item, "ib_tagids") and item.ib_tagids != ""
            else []
        )
